...
@Component({
  selector: 'app-view-incomplete-participants',
  templateUrl: './view-incomplete-participants.component.html',
  styleUrls: ['./view-incomplete-participants.component.css']
})
export class ViewIncompleteParticipantsComponent implements OnInit {
  ...
  getAgenciesList() {
    ...
    getProgramsByAgencyAdmin(agency:any) {
      this.submitedData = ''
      this._commonService.getDataByUrl(`${APIS.programCreation.getIncompletProgramsListByAgency+'/'+agency}`).subscribe({
        next: (res: any) => {
