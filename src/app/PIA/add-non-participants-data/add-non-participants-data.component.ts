...
@Component({
  selector: 'app-add-non-participants-data',
  templateUrl: './add-non-participants-data.component.html',
  styleUrls: ['./add-non-participants-data.component.css']
})
export class AddNonParticipantsDataComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    private route: ActivatedRoute,) { 
     console.log( this._commonService.getOption('mobileNumberForNonParticipant'),'mobileNumberForNonParticipant');
  ...
  ngOnInit(): void {
    ...
    this.formDetails();
     if(this._commonService.getOption('mobileNumberForNonParticipant')){
      ...
      this.getDataByMobileNumber(this._commonService.getOption('mobileNumberForNonParticipant'));
     }
  ...
  formDetails() {
    this.ParticipantDataForm = new FormGroup({
      ...
      isAspirant: new FormControl("Existing Oragnization", ),
  ...
  if(this.isedit){
    ...
    this._commonService
        .add(APIS.nonparticipant.update, payload).subscribe({
      next: (data: any) => {
        if(data?.status==400){
    ...
    let Url=APIS.nonparticipant.add
    if(Object?.keys(this.previousParticipationDetails)?.length){
      Url=APIS.nonparticipant.update
    ...
    this._commonService
      .add(Url, payload).subscribe({
        next: (data: any) => {
          if(data?.status==400){
            this.toastrService.error(data?.message, "Non Participant Data Error!");
          }
          else{
            // this.advanceSearch(this.getSelDataRange);
          // this.getData()
          ...
          this.toastrService.success('Non Participant Data Added Successfully', "Non Participant Data Success!");
          }
      ...
      this._commonService.getById(APIS.captureOutcome.getParticipantData,MobileNumber).subscribe({
        next: (res: any) => {
          ...
          if(res.status==400){
            this.previousParticipationDetails={}
            this.ParticipantDataForm.reset()
            this.ParticipantDataForm.patchValue({mobileNo:MobileNumber,isAspirant:'Existing Oragnization'})
          }
          else{
            let item = res?.data;
