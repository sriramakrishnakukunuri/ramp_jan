...
@Component({
  selector: 'app-training-targets',
  templateUrl: './training-targets.component.html',
  styleUrls: ['./training-targets.component.css']
})
export class TrainingTargetsComponent implements OnInit {
      ...
      this._commonService.getDataByUrl(APIS.trainingtargets.getTrainingtargets+"?agencyId="+value).subscribe({
        next: (dataListResponse: any) => {
          ...
          if(Object.keys(dataList.groupedFinancialTargets ).length) {
            ...
            this.tableheaderList = Object.values(dataList.groupedFinancialTargets).map((item:any) => {
