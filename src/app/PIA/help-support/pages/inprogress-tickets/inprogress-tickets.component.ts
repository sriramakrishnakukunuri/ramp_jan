...
declare var $: any;
@Component({
  selector: 'app-inprogress-tickets',
  templateUrl: './inprogress-tickets.component.html',
  styleUrls: ['./inprogress-tickets.component.css']
})
export class InprogressTicketsComponent implements OnInit {
...
adminUpdateForm!: FormGroup; 
 ...
 getAssigneeName() {
  ...
  loadTickets(): void {
    this.loading = true;
    let url:any=(this.loginsessionDetails?.userRole=='ADMIN' || this.loginsessionDetails?.userRole == 'SPIU')?APIS.tickets.getAll+'?statusFilter=OPEN':APIS.tickets.getDataByReportandStatus+this.loginsessionDetails.userId+'?statusFilter=OPEN'
...
// Initialize the admin update reactive form
  ...
  openTicketDetailsAdmin(ticket: any): void {
    ...
    this.activeScreenTab=''
    // Show the modal
    const modal = new bootstrap.Modal(document.getElementById('ticketDetailModalAdmin'));
    modal.show();
  }
