...
export class SampleUiComponent implements OnInit {
  ...
  ngOnInit(): void {
   ...
   loadDistricts(): void {
    this.isLoadingDistricts = true;
     this._commonService.getDataByUrl(`https://metaverseedu.in/MSME/districts`).subscribe({
      next: (res: any) => {
        this.districts = res || [];
  ...
  loadMandals(districtId: string): void {
    ...
    this._commonService.getDataByUrl(`https://metaverseedu.in/MSME/mandals/${districtId}`).subscribe({
      next: (res: any) => {
        this.mandals = res || [];
  ...
  loadVillages(mandalId: string): void {
    ...
    this._commonService.getDataByUrl(`https://metaverseedu.in/MSME/villages/${mandalId}`).subscribe({
      next: (res: any) => {
        this.villages = res || [];
  ...
  loadUnits(villageId: string): void {
    ...
    this._commonService.getDataByUrl(`https://metaverseedu.in/MSME/units?village=${villageId}&madal=${mandalname}`).subscribe({
      next: (res: any) => {
        this.units = res || [];
